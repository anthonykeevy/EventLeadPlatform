"""fix_company_viewer_no_default_access

Revision ID: 96cfcbf141d3
Revises: a2421aeb293d
Create Date: 2025-11-20 16:54:42.138930

Story: Epic 2 - Form Access Control Bug Fix
Purpose: Remove company_viewer from company role defaults - Company Viewers should only get access through explicit FormAccessControl entries

Changes:
1. Remove company_viewer from Priority 5 (Company Role Default) in fn_GetUserFormAccess
2. Company Viewers now only get access through Priority 3 (Explicit FormAccessControl) or Priority 4 (Agency Event-Scoped Access)
3. If no explicit access, Company Viewers get Priority 6 (No Access)
...
revision = '96cfcbf141d3'
down_revision = 'a2421aeb293d'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Remove company_viewer from company role defaults in fn_GetUserFormAccess"""
    
    # Drop and recreate the function without company_viewer in Priority 5
    op.execute("DROP FUNCTION IF EXISTS [dbo].[fn_GetUserFormAccess]")
    
    # Recreate function without company_viewer in company role defaults
    op.execute("""
...
def downgrade() -> None:
    """Revert fix - restore company_viewer to company role defaults"""
    
    # Drop function
    op.execute("DROP FUNCTION IF EXISTS [dbo].[fn_GetUserFormAccess]")
    
    # Recreate function with company_viewer in Priority 5 (original version)
    op.execute("""
        CREATE FUNCTION [dbo].[fn_GetUserFormAccess]
        (
            @UserID BIGINT,
        ...
        (
            ...
            SELECT
                ...
                CASE
                    ...
                    -- Priority 5: Company Role Default (reverted - includes company_viewer)
                    WHEN UserCompanyRole = 'company_admin' THEN
                        (SELECT FormAccessControlAccessTypeID FROM ref.FormAccessControlAccessType WHERE AccessTypeCode = 'MANAGE')
                    WHEN UserCompanyRole = 'company_user' THEN
                        (SELECT FormAccessControlAccessTypeID FROM ref.FormAccessControlAccessType WHERE AccessTypeCode = 'VIEW')
