"""Fix Agency Access Join Issue - Use Subqueries Instead of LEFT JOINs

Revision ID: fix_agency_access_join_issue
Revises: 1d6fd98cc9ea
Create Date: 2025-11-21 12:00:00.000000

Story: Epic 2 - Form Access Control Enhancement
Purpose: Fix fn_GetUserFormAccess to correctly detect agency relationships when user is in multiple companies

Problem:
When a user is in multiple companies, the LEFT JOIN on UserCompany for agency check creates multiple rows,
and the function may return a row with NULL agency fields instead of finding the agency relationship.

Solution:
Replace LEFT JOINs with subqueries that use MAX() to find any agency relationship across all UserCompany records.
This ensures the function correctly detects agency access regardless of which UserCompany record is considered.
...
revision = 'fix_agency_access_join_issue'
down_revision = '1d6fd98cc9ea'
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Fix agency access check to use subqueries instead of LEFT JOINs"""
    
    # Drop and recreate the function with fixed agency check logic
...
def downgrade() -> None:
    """Revert to previous version (with LEFT JOINs)"""
    
    # Drop function
    op.execute("DROP FUNCTION IF EXISTS [dbo].[fn_GetUserFormAccess]")
    
    # Recreate with LEFT JOINs (previous version)
    # This reverts to the version in migration 1d6fd98cc9ea
    op.execute("""
        CREATE FUNCTION [dbo].[fn_GetUserFormAccess]
        (
            @UserID BIGINT,
