...
async def get_user_companies(
    ...
    """
    Get all active companies user belongs to (AC-1.7.9).
    ...
    Returns:
        List of active UserCompany relationships
    """
...
async def switch_company(
    ...
    Args:
        db: Database session
        user_id: User ID
        target_company_id: Company ID to switch to
        
    Returns:
        UserCompany relationship
        
    Raises:
        ValueError: If user doesn't belong to company
    """
    ...
    if not user_company:
        raise ValueError("You do not belong to this company")
    
    # Log company switch to audit (AC-1.7.10)
    audit_log = ActivityLog(
        UserID=user_id,
        CompanyID=target_company_id,
        Action="COMPANY_SWITCHED",
        EntityType="UserCompany",
        EntityID=user_company.UserCompanyID,
        NewValue=json.dumps({
    ...
    logger.info(
        f"Company switched: UserID={user_id}, NewCompanyID={target_company_id}"
    )
