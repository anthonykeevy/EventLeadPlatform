...
| **Unified Form Approval Workflow** | Single approval workflow with multiple gates (RequirePublishApproval, EnforceDemoTestRequirement, FormCostThreshold). Unify cost gate with publish approval; deprecate separate FormApprovalStatus path. See `docs/UNIFIED-APPROVAL-WORKFLOW-IDEA.md`. | User feedback 2026-02-20 |
| **Event–Form Workflow Consistency** | Align Event and Form workflows for consistency and clear messaging. **Event Status:** Users are told "Event needs to be in a published state before their Forms will be active" (Event statuses: Draft, Pending Review, Published, Completed, Cancelled). Form activation currently uses StartDate/EndDate only—not Event Status. Align logic with messaging or vice versa. **Event approval workflow:** Event has approval flow but no Company-level config (unlike Forms with CompanyFormTestConfig). Add Company properties for Event approval rules to match Form Approval Workflow pattern. Ensures consistent customer experience and avoids confusion. | User feedback 2026-02-20 |
...
*Last Updated: 2026-02-23 — Story 5.8 complete; 5.9 next*
