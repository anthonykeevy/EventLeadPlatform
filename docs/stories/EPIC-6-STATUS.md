...
| **6.5c** | **Capability Catalog Cutover** | Make `resolve_allowed_components` authoritative for Blocks A/F/I AND the frontend toolbox (via `/api/form-builder/init`). Migrate Block F (COMPONENT_CAPABILITY) into the registry as GENERATED + prose-shell. Replace `brandPosture` enum with `ref.BrandPosture` and re-wire Block C variant selection through it. Isolated into its own story because the toolbox/capability cutover is the highest-risk single change in the prompt-assembly roadmap. | 🟡 **Ready for Dev** (2026-05-20) — PR [#106](https://github.com/anthonykeevy/EventLeadPlatform/pull/106) Draft → `develop`; story pack in worktree |
...
*Last Updated: 2026-05-20 — Story 6.5c **Ready for Dev** (PR #106 Draft). Story 6.5b **Complete** (PR #104 + #105). **Current implementation focus:** Story 6.5c — Capability Catalog Cutover.*  
