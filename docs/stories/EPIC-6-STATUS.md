...
| **6.5a** | **Clarification Questions** | Situational awareness dropdowns (Audience Locale, Form Purpose, Respondent Type) injected as replaceable prompt Block E; replaces hardcoded `AudienceLocale` enum with `ref.AudienceLocale` registry table. **Also resolves the Azure-only `context-pack-load-failed` bug** by moving the prompt context from `docs/stories/STORY-6.2-AI-CONTEXT-PACK.md` (filesystem) into the database registry — the file lives outside the `backend/` deploy package, so it never reaches `/home/site/wwwroot/` and `Path(__file__).parents[3]` resolves to `/home/site/` on Azure (where `docs/` does not exist). Today AI form generation works locally but fails immediately in the Azure Test slot with `terminal: context-pack-load-failed`; not a regression vs `master` (Epic 6 features are develop-only). **Data model + prompt-assembly registry architecture approved by Dimitri (Rev 9, 2026-05-09)** — see `docs/architecture/decision-6.5a-clarification-options-data-model.md` and `docs/architecture/prompt-assembly-registry-architecture.md`. | 🔄 **Architecture phase complete — Draft PR #87 open.** Implementation pending; first story to ship under the new Environment Promotion Workflow (see `EPIC-6-WORKFLOW-GUIDE.md`). PR #87 to be re-targeted from `master` → `develop` when the new workflow lands. |
...
| **6.11** | **Production Environment + CI/CD + Manual Approval Gate** | Provision the Azure production resources (App Service production slot or dedicated plan, Azure SQL prod, Blob, ACS verified sender domain, Application Insights, Key Vault) per `docs/architecture/azure-infrastructure-architecture.md`. Add `.github/workflows/deploy-to-prod.yml` triggered on push to `master` with a **manual-approval gate** on the `production` GitHub Environment. Configure custom domain + Cloudflare DNS for `app.signalplatforms.io`. Validate Stripe production credentials end-to-end. Run cutover smoke checks. **Sequenced last so production opens with billing live (6.6–6.10).** Closes Phase D candidate D1 (production-side). | ⏳ Pending (scheduled post-6.10) |
...
*Last Updated: 2026-05-19 — (1) Story 6.5a row updated: Dimitri's data-model + prompt-assembly registry architecture approved (Rev 9, 2026-05-09), Draft PR #87 open, implementation pending. (2) Added Story 6.11 — Production Environment + CI/CD + Manual Approval Gate — at the end of Phase B (sequenced post-6.10 so production opens with billing live). (3) Phase D row D1 split: Test environment ✅ done (Azure + GitHub Actions `deploy-to-test.yml` on `develop`); Production work promoted to Story 6.11. (4) See `EPIC-6-WORKFLOW-GUIDE.md` for the new Environment Promotion Workflow (Worktree → develop → master).*