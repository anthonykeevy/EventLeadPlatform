...
| **6.5b** | **Prompt Assembly Registry Foundation** *(Closes R6)* | Stand up `PromptAssemblyProfile*` / `PromptSection*` schema + Python resolver/renderer. Migrate stored-prose Blocks A (ROLE_CONTRACT), B (SAFETY), C (BRAND_POSTURE — 4 variants), G (FEW_SHOT — the context-pack), I (JSON_OUTPUT) from code into seeded DB variants with variant-level versioning. **Block G migration moves `STORY-6.2-AI-CONTEXT-PACK.md` content into `PromptSectionVariant.PromptSnippet` — eliminates the Azure file-load failure and closes R6 from PR #101.** Frontend unchanged; equivalence test guards behaviour. AC-19 prompt-equivalence diff is Tony's sign-off gate before PR moves Draft → Ready. §5a Local Validation Flow lets Dev iterate locally without Azure cycle penalty. See `story-6.5b.md`. | 🔄 **Ready for Dev** (drafted 2026-05-20) |
