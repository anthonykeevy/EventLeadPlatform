...
| **6.5d** | **Clarification Data Plane + Component Catalog Completion** | **Track A:** Seed missing `FormBuilderComponent` rows (`rating`, `url`, `file-upload`, `paragraph`, `address`); AU `address-lookup-au`; `ADD-COMPONENT-TO-PLATFORM-CHECKLIST.md`; `verify_component_catalog_alignment.py`. **Track B:** Three `ref.*` tables + APIs; Block E registry; AI panel dropdowns; remove `AudienceLocale` enum; Company/Form/GenerationRun persistence. PR [#109](https://github.com/anthonykeevy/EventLeadPlatform/pull/109) Draft. | 🔄 **Ready for Dev** (2026-05-21) — worktree `C:\wt\elp\story-epic6-6.5d-clarification-component-platform` |
