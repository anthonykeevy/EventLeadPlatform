...
| **6.5a** | **Clarification Questions** | Situational awareness dropdowns (Audience Locale, Form Purpose, Respondent Type) injected as replaceable prompt Block E; replaces hardcoded `AudienceLocale` enum with `ref.AudienceLocale` registry table. **Data model + prompt-assembly registry architecture approved by Dimitri (Rev 9, 2026-05-09)** — see `docs/architecture/decision-6.5a-clarification-options-data-model.md` and `docs/architecture/prompt-assembly-registry-architecture.md`. | 🔄 **Architecture phase complete — Draft PR #87 open.** Implementation pending; first story to ship under the new Environment Promotion Workflow (see `EPIC-6-WORKFLOW-GUIDE.md`). PR #87 to be re-targeted from `master` → `develop` when the new workflow lands. |
