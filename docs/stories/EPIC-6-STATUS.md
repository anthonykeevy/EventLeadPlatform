...
| **6.5c** | **Capability Catalog Cutover** | Make `resolve_allowed_components` authoritative for Blocks A/F/I AND the frontend toolbox (via `/api/form-builder/init`). Migrate Block F (COMPONENT_CAPABILITY) into the registry as GENERATED + prose-shell. Replace `brandPosture` enum with `ref.BrandPosture` and re-wire Block C variant selection through it. Isolated into its own story because the toolbox/capability cutover is the highest-risk single change in the prompt-assembly roadmap. | ⏳ **Pending** — unblocked (6.5b complete) |
...
*Last Updated: 2026-05-20 — Story 6.5b **Complete** (PR #104 merged to `develop`). LocalDB + Azure Test UAT green; **R6 closed** (`context-pack-load-failed` no longer reproduces). Migrations `078`–`083`. **Current implementation focus:** Story 6.5c — Capability Catalog Cutover.*  
