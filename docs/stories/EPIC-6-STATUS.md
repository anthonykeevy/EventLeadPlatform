...
| **6.4.4.2** | **Re-evaluate H2/H4 under rubric_v2** | Ablation study: H2-only and H4-only re-run against the AC10 `rubric_v2` baseline; both failed the ship bar, so current `master` behavior remains unchanged. | ✅ **Review / measured no-change** (PR #79) |
