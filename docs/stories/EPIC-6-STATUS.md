...
| **6.4.6** | **AU-Only Diagnostic Evaluation Framework + Baseline** | Build the AU-only diagnostic framework, rewrite/tag benchmark prompts for AU launch context, add prompt-context linter/shared context bundles/judge diagnostics/deterministic AU checks, and produce the first current-state AU baseline. | 🚧 **In progress** (Draft PR #82) |
