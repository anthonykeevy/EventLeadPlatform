...
| **6.5b** | **Prompt Assembly Registry Foundation** *(Closes R6)* | Stand up `PromptAssemblyRegistry*` / `PromptSection*` / `PromptSectionVariant*` / `PromptSectionData` schema + Python resolver/renderer (`backend/modules/form_ai/prompt_assembly/`). Migrated Blocks A (ROLE_CONTRACT), B (SAFETY), C (BRAND_POSTURE — 4 variants), G (FEW_SHOT — context-pack), I (JSON_OUTPUT) from code/disk into seeded DB variants with variant-level versioning. Block G migration (`081_story_6_5b_seed_block_g_context_pack.py`) inlines the `STORY-6.2-AI-CONTEXT-PACK.md` prose into `PromptSectionVariant.PromptSnippet` and removes `_load_context_pack` — closes **R6** (`context-pack-load-failed` on the Azure Test environment). `dbo.GenerationRun` extended with `PromptAssemblyRegistryVersionID` + `PromptVariantSnapshot` for replayability. Frontend unchanged; AC-19 byte-equivalence diff (`STORY-6.5b-PROMPT-EQUIVALENCE-DIFF.md`) **PASS** across all four brand postures (Tony sign-off pending). See `story-6.5b.md`. | 🟡 **Ready for UAT** (Dev complete 2026-05-20, PR #104) — Tony to sign off equivalence diff + run alembic upgrade head; status flips ✅ when verified resolved on Test. |
| **6.5c** | **Capability Catalog Cutover** | Make `resolve_allowed_components` authoritative for Blocks A/F/I AND the frontend toolbox (via `/api/form-builder/init`). Migrate Block F (COMPONENT_CAPABILITY) into the registry as GENERATED + prose-shell. Replace `brandPosture` enum with `ref.BrandPosture` and re-wire Block C variant selection through it. Isolated into its own story because the toolbox/capability cutover is the highest-risk single change in the prompt-assembly roadmap. | ⏳ **Pending** — depends on 6.5b |
| **6.5d** | **Clarification Data Plane** | The original 6.5a content — but now plugging into the registry from 6.5b. Three `ref.*` tables (`AudienceLocale` with `ClarificationSummary`, `FormPurpose`, `RespondentType`) seeded for the 11 existing audience locales / 10 purposes / 9 respondents. Three reference APIs returning `(list + resolved default)`. Block E (E1/E2/E3) injected via registry. Frontend dropdowns in AI Agent panel; `AudienceLocale` TypeScript enum deleted. Company / Form / GenerationRun schema additions for defaults + snapshot + audit. | ⏳ **Pending** — depends on 6.5b (and benefits from 6.5c's `ref.BrandPosture` precedent) |
| **6.5e-vision** *(was 6.5b-vision — renamed 2026-05-20)* | **Image-to-Form Vision Path** | Screenshot/photo input to `FormSemanticPlan`, reusing deterministic compiler architecture. | ⏳ Pending |
...
*Last Updated: 2026-05-20 (PM) — Story 6.5b Dev complete on PR #104. AC-19 prompt-equivalence diff PASS across all four brand postures (`docs/stories/STORY-6.5b-PROMPT-EQUIVALENCE-DIFF.md`); 6.5b moves to Ready for UAT awaiting Tony's equivalence-diff sign-off + alembic execution + Azure Test verification of R6 closure. Migrations 078-082 prepared; Tony executes via the Story 6.5b migration handoff doc.*  
*Last Updated: 2026-05-20 (AM) — Story 6.5a closed as Architecture Phase; implementation decomposed into 6.5b Registry Foundation [closes R6], 6.5c Capability Catalog Cutover, 6.5d Clarification Data Plane. Speculative stories 6.5b-vision / 6.5b-style / 6.5c / 6.5d renamed to 6.5e-vision / 6.5f-style / 6.5g-PII / 6.5h-fonts to free up the b/c/d slots. PR #87 closed-as-superseded; merged via PR #103.*  
