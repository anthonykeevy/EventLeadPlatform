...
**Story 6.5b Status:** 🟡 **Ready for UAT** (Dev complete 2026-05-20, PR #104) — Prompt Assembly Registry foundation; closes R6 by migrating Block G context-pack into `config.PromptSectionVariant`. AC-19 prompt-equivalence diff PASS across all four brand postures (`docs/stories/STORY-6.5b-PROMPT-EQUIVALENCE-DIFF.md`). Tony to sign off the equivalence diff, run `alembic upgrade head` (migrations 078–082), validate `GenerationRun.PromptAssemblyRegistryVersionID` populates locally, then deploy + verify R6 closure on Test before status flips ✅.
