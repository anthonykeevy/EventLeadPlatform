...
import { FormDefinition, FormComponent, GlobalStyles, DEFAULT_GLOBAL_STYLES, StyleOverrides, LogicRule, FormPage, type AiAgentSettings } from '../types/builder.types';
...
interface BuilderState {
    ...
    /**
     * Story 6.3.1 — currently previewed device tab (desktop / tablet / mobile).
     * Lifted out of FormBuilderCanvas local state so the AI Agent panel can read
     * it when building runtimeContext.canvas — i.e. the AI compiles for the
     * canvas size the user is actively looking at, not always desktop.
     */
    ...
    /** Story 6.3: Persist AI Agent prompt and options on the form definition. */
    setAiAgentSettings: (partial: AiAgentSettings) => void;
...
function formDefinitionFromInit(
    ...
    return {
        ...
        canvasSettings,
        pages,
        ...(def.aiAgentSettings ? { aiAgentSettings: def.aiAgentSettings } : {}),
    } as FormDefinition;
}
...
export const useBuilderStore = create<BuilderState>((set, get) => ({
    ...
    applyValidatedDefinition: (definition, description = 'Apply AI generated layout') => {
        ...
        set((state) => {
            ...
            const currentCanvas = state.formDefinition.canvasSettings;
            const incomingCanvas = definition.canvasSettings;
            // Story 6.3.1 UAT round 4 — apply BOTH width and height from the
            // incoming definition. The earlier "preserve current width" rule
            // was added when the backend would invent its own canvas; now the
            // frontend explicitly tells the compiler which device canvas to
            // target (see AIAgentPanel.buildRuntimeContext → previewMode), so
            // the returned width is the user's chosen width. Locking the width
            // to the previous value here was why "switch to mobile and re-run"
            // produced a 375-wide layout placed inside a 1920-wide canvas
            // (components clustered on the left, off-canvas on the right).
            //
            // We still keep all other canvas settings (gridSize, snapToGrid,
            // backgroundColor, etc.) from the current definition because the
            // compiler doesn't author those — only width/height/gridSize.
            const resolvedCanvasSettings = {
                ...(currentCanvas ?? {}),
                width:
                    incomingCanvas?.width ?? currentCanvas?.width,
                // Height is special: the compiler can grow it, but if the
                // incoming canvas is smaller (e.g. user switched from a long
                // form to a short one) we still trust the incoming value.
                height:
                    incomingCanvas?.height ?? currentCanvas?.height,
            };
            // Story 6.3.1 (UAT round 6) — Fix C: forward
            // ``globalStyles.horizontalLabelBandPx`` from an AI-compiled
            // definition into the preserved global styles. The compiler
            // computes this from the longest label in the semantic plan and
            // emits it on the response so every component lines up at the
            // same input left-edge in horizontal-stacked grid mode. We
            // intentionally allow only this one knob through so the rest of
            // the user's form-level styling (theme, colors, fonts, gaps) is
            // untouched by AI generation — the compiler is opinionated about
            // layout, not about look-and-feel.
            const incomingGlobalStyles = (definition.globalStyles ?? null) as Record<string, unknown> | null;
            const aiHorizontalLabelBandPx = incomingGlobalStyles?.horizontalLabelBandPx;
            const mergedGlobalStyles =
                typeof aiHorizontalLabelBandPx === 'number' && Number.isFinite(aiHorizontalLabelBandPx)
                    ? {
                          ...state.formDefinition.globalStyles,
                          horizontalLabelBandPx: aiHorizontalLabelBandPx,
                      }
                    : state.formDefinition.globalStyles;
            ...
            const incoming = {
                ...
                canvasSettings: resolvedCanvasSettings,
                aiAgentSettings: state.formDefinition.aiAgentSettings,
            };
    ...
    setAiAgentSettings: (partial) => {
        set((state) => {
            if (!state.formDefinition) return state;
            const prev = state.formDefinition.aiAgentSettings ?? {
                includeEventInformation: true,
            };
            return {
                formDefinition: {
                    ...state.formDefinition,
                    aiAgentSettings: { ...prev, ...partial },
                },
            };
        });
        persistToStorage(get().formDefinition);
