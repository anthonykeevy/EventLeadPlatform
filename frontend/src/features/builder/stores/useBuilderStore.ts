    ...
    applyValidatedDefinition: (definition, description = 'Apply AI generated layout') => {
        ...
        set((state) => {
            ...
            const resolvedCanvasSettings = {
                ...(currentCanvas ?? {}),
                // Keep current width/grid authoring context, but accept any height expansion
                // generated by backend fit-to-canvas guardrails.
                height: Math.max(
                    currentCanvas?.height ?? 0,
                    incomingCanvas?.height ?? 0
                ) || currentCanvas?.height || incomingCanvas?.height,
            };
            const incoming = {
                ...definition,
